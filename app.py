...
def preprocessing():
    ...
    dtvw =  data_cleaning.head()
    data_dsc =data_cleaning.isna().sum()
    total_data = len(data_cleaning)
    data_dsc = {"Data Kosong " : data_dsc}
    data_dsc = pd.DataFrame(data_dsc)
    dict_obj = data_cleaning.to_dict('list')
    session['data_cleaning'] = dict_obj

    # return render_template('show_data.html', data_var = uploaded_df_html)
    return render_template('preprocessing.html',tables=[data_cleaning.to_html(classes='data')], titles=uploaded_df.columns.values,dts=[data_dsc.to_html(classes='data')],tot = total_data  )
...
def apriori():
    ...
    # uploaded_df = uploaded_df
    dtvw = data_group.head()
    return render_template('apriori.html',tables=[dtvw.to_html(classes='data')], titles=dtvw.columns.values)
...
def apriori_refresh():
    ...
    for item in association_results:
        ...
        ds = {'Item1': f"{items[0]}", "Item2": f"{items[1]}", 'Support': f"{str(round(item[1], 3))}", 'Confidence:': f"{str(round(item[2][0][2], 3))}", 'lift': f"{str(round(item[2][0][3], 3))}"}
